...
describe('validateEmailAddress', () => {
  ...
  it('should return valid result for valid email addresses', () => {
    ...
    expect(validateEmailAddress('john.doe@company.co.uk', disposableDomains)).toEqual({
      isValid: true,
      email: 'john.doe@company.co.uk',
      error: ''
    })
    expect(validateEmailAddress('name+tag@gmail.com', disposableDomains)).toEqual({
      isValid: true,
  ...
  it('should return invalid result for invalid email format', () => {
    ...
    expect(validateEmailAddress('@missingusername.com', disposableDomains)).toEqual({
      isValid: false,
      email: '@missingusername.com',
      error: 'Invalid email format'
    })
    expect(validateEmailAddress('spaces in@email.com', disposableDomains)).toEqual({
      isValid: false,
  ...
  it('should return invalid result for disposable email addresses', () => {
    expect(validateEmailAddress('user@disposable.com', disposableDomains)).toEqual({
      isValid: false,
      email: 'user@disposable.com',
      error: 'Disposable email addresses are not allowed'
    })
    expect(validateEmailAddress('test@temporary.net', disposableDomains)).toEqual({
      isValid: false,
