...
export function validateAndTransformString(
  ...
  if (config.minLength && value.length < config.minLength) {
    error = config.errorMessages?.length || `Minimum length is ${config.minLength}`;
    return { value, error };
  }

  if (config.maxLength && value.length > config.maxLength) {
    error = config.errorMessages?.length || `Maximum length is ${config.maxLength}`;
    return { value, error };
  }

  if (config.exactLength && value.length !== config.exactLength) {
    error = config.errorMessages?.length || `Exact length must be ${config.exactLength}`;
    return { value, error };
  }
      ...
      recordHook(config.sheetSlug || '**', (record) => {
        for (const field of config.fields) {
          ...
          if (value !== null && value !== undefined) {
            const { value: newValue, error } = validateAndTransformString(value, config);
            if (error) {
