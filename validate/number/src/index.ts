          ...
          for (const field of config.fields) {
            ...
            try {
              ...
              if (config.integerOnly && !Number.isInteger(numberValue)) {
                throw new Error(`The field '${field}' must be an integer`)
              }
              ...
              if (config.specialTypes) {
                if (
                  ...
                  throw new Error(`The field '${field}' must be a prime number`)
                }
                if (
                  config.specialTypes.includes('even') &&
                  numberValue % 2 !== 0
                ) {
                  throw new Error(`The field '${field}' must be an even number`)
                }
                if (
                  config.specialTypes.includes('odd') &&
                  numberValue % 2 === 0
                ) {
                  throw new Error(`The field '${field}' must be an odd number`)
                }
              }

              logInfo(
                'number-validation',
                `Number validation passed for field '${field}'`
              )
            } catch (error) {
              logError(
                'number-validation',
