import { validateAmount, validateDate, convertCurrency, calculateExchangeRate, fetchExchangeRates } from './convert.currency.plugin';
...
describe('Currency Converter Plugin Utility Functions', () => {
  describe('validateAmount', () => {
    ...
    it('should return an error for invalid input', () => {
      ...
      expect(validateAmount('abc')).toEqual({ error: 'Amount must be a valid number' });
    });
  });
  ...
  describe('validateDate', () => {
    ...
    it('should return an error for invalid date format', () => {
      expect(validateDate('05-01-2023')).toEqual({ error: 'Invalid date format. Use YYYY-MM-DD' });
    });
  });
  ...
  describe('fetchExchangeRates', () => {
    ...
    it('should fetch historical exchange rates when a date is provided', async () => {
      ...
      const result = await fetchExchangeRates(mockApiKey, mockSourceCurrency, mockTargetCurrency, mockDate);

      expect(fetch).toHaveBeenCalledWith(expect.stringContaining('historical/2023-05-01.json'));
      expect(result).toEqual({
        rates: { USD: 1, EUR: 0.83 },
        base: 'USD',
        timestamp: 1682899200,
    ...
    it('should throw an error when the API request fails', async () => {
      ...
      await expect(fetchExchangeRates(mockApiKey, mockSourceCurrency, mockTargetCurrency))
