...
export class psvExtractor extends AbstractExtractor {
  ...
  public parseBuffer(buffer: Buffer): Record<string, SheetCapture> {
    const workbook = psv2json.read(buffer, {
      type: "buffer",
      cellDates: true,
    });
   ...
   * @param sheet
   */
  convertSheet(sheet: psv2json.WorkSheet): SheetCapture {
    let rows = psv2json.utils.sheet_to_json<Record<string, any>>(sheet, {
      header: "A",
      defval: null,
    });
  ...
  /**
   * Extract the data from an uploaded PSV file
   */
