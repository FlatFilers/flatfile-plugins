...
describe('multi-file utils', () => {
  ...
  describe('generateFilePriority', () => {
    ...
    it('should prioritize exact match over pattern', () => {
      ...
      const priority = generateFilePriority('test_sheet', { 'test_sheet': 10 }, patterns)
      expect(priority).toBe(10)
    })
  })
  ...
  describe('createProviderTrustMatcher', () => {
    ...
    const createRecord = (id: string, externalId: string, npi: string, ssn: string): RecordWithSource => ({
      id,
      values: {
  ...
  describe('createProviderTrustMerger', () => {
    ...
    const createRecord = (id: string, externalId: string, firstName: string, npi: string, priority: number, sheetName: string): RecordWithSource => ({
      id,
      values: {
        external_id: { value: externalId },
    ...
    it('should prioritize populated core fields from higher priority source', () => {
      ...
      const record2 = createRecord('2', 'ext123', 'Gary', '6863666149', 10, 'Peoplesoft')
    ...
    it('should fill missing core fields from lower priority sources', () => {
      const record1 = createRecord('1', 'ext123', 'Gary', '6863666149', 5, 'MDStaff')
