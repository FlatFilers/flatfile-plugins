  ...
  for(const workbook of workbooks) {
    ...
    for(const sheet of sheets) {
      ...
      const fieldTable = fields.map(field => {
        const constraints = field.constraints?.map(constraint => `<span style="color: #d97a71; background-color: #fff0ef; padding: 2px 6px; border-radius: 5px; margin-left: 8px;">${constraint.type}</span>`).join('');
        return `
  <tr>
