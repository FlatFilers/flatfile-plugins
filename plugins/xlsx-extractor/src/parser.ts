...
/**
 * Streaming version of parseBuffer that returns a StreamingParseResult
 */
...
/**
 * Convert a sheet to streaming format
 */
async function convertSheetToStreaming({
  sheet,
  sheetName,
  rawNumbers,
  raw,
...
> {
  ...
  // Create an async generator for the records
  async function* generateRecords() {
    for (const row of rowsAsArrays) {
      const record = row.reduce((acc, value, index) => {
