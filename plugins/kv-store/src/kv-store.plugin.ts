  ...
  set: async (key: string, value: any): Promise<void> => {
    ...
    try {
      const response = await fetch(
        `${process.env.FLATFILE_KV_URL}/key/${key}`,
        {
          method: 'POST',
      ...
      throw new Error(
        `KV Store: Failed to set value '${value}' to key '${key}'. Error: ${error}`
      )
    }
  },
  get: async (key: string): Promise<any> => {
    ...
    try {
      ...
      try {
        ...
        throw new Error(
          `KV Store: Failed to parse value '${data}' for key '${key}'. Error: ${error}`
        )
      }
    } catch (error) {
      throw new Error(
        `KV Store: Failed to get value for key '${key}'. Error: ${error}`
      )
    }
  },
  ...
  list: {
    append: async (
      ...
      try {
        ...
        if (Array.isArray(currentValue)) {
          if (options.unique) {
            ...
            const newValues = values.filter(
              (value: any) => !currentValue.includes(value)
            )
        ...
        throw new Error(
          `KV Store: Failed to set values '${values}' to key '${key}'. Error: ${error}`
        )
      }
    },
    delete: async (key: string, values: any[]): Promise<void> => {
      try {
        ...
        throw new Error(
          `KV Store: Failed to delete values '${values}' from key '${key}'. Error: ${error}`
        )
      }
    },
    pop: async (key: string): Promise<any> => {
      try {
        ...
        throw new Error(
          `KV Store: Failed to pop from key '${key}'. Error: ${error}`
        )
      }
    },
    shift: async (key: string): Promise<any> => {
      try {
        ...
        throw new Error(
          `KV Store: Failed to shift from key '${key}'. Error: ${error}`
        )
      }
    },
  },
